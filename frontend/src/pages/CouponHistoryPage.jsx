  ...
  return (
    <div className="flex justify-center">
      <div className="w-[520px] flex flex-col gap-4">
        <Link to={`/my-coupons/${couponIssueId}`} className="text-[13px] text-sub">
          &lt; 쿠폰 상세
        </Link>
        <div className="text-lg font-bold text-ink">{detailQ.data?.title} · 이력</div>

        <div className="flex flex-col">
          {entries.length === 0 && <div className="text-sm text-sub py-8 text-center">이력이 없어요</div>}
          {entries.map((h, i) => {
            ...
            return (
              <div key={i} className="flex gap-3">
                <div className="flex flex-col items-center">
                  ...
                  {!isLast && <div className="w-px flex-grow bg-line" />}
                </div>
                <div className="pb-6 flex-grow">
                  <div className="text-[11px] text-sub">{fmtDateTime(h.createdAt)}</div>
                  <div className="text-[13px] font-semibold text-ink mt-0.5">
                    {TRANSITION_LABEL[key] ?? `${h.prevStatus} → ${h.newStatus}`}
                  </div>
                  {h.cancelReason && (
                    <div className="text-xs text-sub mt-1 border border-line rounded-md p-2">
                      사유: {h.cancelReason}
                    </div>
                  )}
                </div>
